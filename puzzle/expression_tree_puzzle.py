...
class ExpressionTreePuzzle(Puzzle):
    ...
    def is_solved(self) -> bool:
        ...
        for v in self.variables:
            if self.variables[v] == 0 or not 1 <= self.variables[v] <= 9:
                return False
        return self.target == self._tree.eval(self.variables)

    # (Task 5) override __str__
        ...
        for key in self.variables:
            if self.variables[key] == 0:
                for i in range(1, 10):
                    ...
                    new_dic[key] = i
                    new = ExpressionTreePuzzle(tree, self.target)
                    new.variables = new_dic
                    lst.append(new)
        return lst

    # (TASK 5): override fail_fast
    ...
    def fail_fast(self) -> bool:
        ...
        """
        tree = self._tree.copy()
        dic1 = self.variables.copy()
        dic2 = self.variables.copy()
        for v in dic1:
            if dic1[v] == 0:
                dic1[v] = 1
        for v in dic2:
            if dic2[v] == 0:
                dic2[v] = 9
        minimum = tree.eval(dic1)
        maximum = tree.eval(dic2)
        return not minimum <= self.target <= maximum
