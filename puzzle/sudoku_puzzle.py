...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for i in range(self._n):
            for j in range(self._n):
                row = {self._grid[i][j]} | self._row_set(i)
                col = {self._grid[i][j]} | self._column_set(j)
                sub = {self._grid[i][j]} | self._subsquare_set(i, j)
                if " " not in row and row != self._symbol_set:
                    return True
                if " " not in col and col != self._symbol_set:
                    return True
                if " " not in sub and sub != self._symbol_set:
                    return True
                if self._grid[i][j] == EMPTY_CELL:
                    combine = self._row_set(i) | self._column_set(j)
                    combine = combine | self._subsquare_set(i, j)
                    combine.remove(" ")
                if combine == self._symbol_set:
                    return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        solver1 = DfsSolver()
        solver2 = BfsSolver()
        solution1 = solver1.solve(self)
        if solution1:
            solution1 = solution1[-1]
        solution2 = solver2.solve(self)
        if solution2:
            solution2 = solution2[-1]
        if solution1 != solution2:
            return False
        seen = set()
        seen.add(str(solution1))
        if solver1.solve(self, seen):
            return False
        if solver2.solve(self, seen):
            return False
        return True
