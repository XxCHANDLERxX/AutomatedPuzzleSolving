...
class BfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        while not q.is_empty():
            curr = q.dequeue()
            for p in curr[-1].extensions():
                if str(p) not in seen and p.is_solved():
                    curr += [p]
                    break
                if str(p) not in seen and not p.fail_fast():
                    q.enqueue(curr + [p])
                    seen.add(str(p))
            if curr[-1].is_solved():
                path = curr
                break
        if q.is_empty() and not path[-1].is_solved():
            return []
        else:
            return path
