...
class WordLadderPuzzle(Puzzle):
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        True
        >>> wl = WordLadderPuzzle("chimps", "report", load_words())
        >>> solver = BfsSolver()
        >>> wl_solu = solver.solve(wl)
        >>> for index in wl_solu: print(index)
        """
        # lst = []
        # for i in range(len(self.from_word)):
        #     for letter in LETTERS:
        #         word = self.from_word[:i] + letter + self.from_word[i + 1:]
        #         if word in self.word_set and word != self.from_word:
        #             new = WordLadderPuzzle(word, self.to_word, self.word_set)
        #             if self._helper(word) <= 2:
        #                 lst.insert(0, new)
        #             else:
        #                 lst.append(new)
        # return lst
        ...
        for i in range(len(self.from_word)):
            for letter in LETTERS:
                ...
                if word in self.word_set and word != self.from_word:
                    lst.append(word)
        lst.sort(key=self._helper)
        lst1 = []
        for w in lst:
            new = WordLadderPuzzle(w, self.to_word, self.word_set)
            lst1.append(new)
        return lst1

    def _helper(self, word1: str) -> int:
        """
        Return the number of digit difference between word1 and word2.
        """
        count = 0
        for i in range(len(word1)):
            if word1[i] != self.to_word[i]:
                count += 1
        return count
    ...
    def get_difficulty(self) -> str:
        ...
        if self.is_solved() or len(solution) == 2:
            return TRIVIAL
        elif len(solution) == 3:
            return EASY
