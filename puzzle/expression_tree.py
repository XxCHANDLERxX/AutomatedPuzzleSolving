...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if self._root == OP_ADD:
            return sum([x.eval(lookup) for x in self._subtrees])
        elif self._root == OP_MULTIPLY:
            res = 1
            for tree in self._subtrees:
                res *= tree.eval(lookup)
            return res
        else:
    ...
    def __str__(self) -> str:
        ...
        elif self._root == OP_ADD or self._root == OP_MULTIPLY:
            output = '('
            for i in range(len(self._subtrees)):
                output += str(self._subtrees[i])
                if i != len(self._subtrees) - 1:
                    output = output + ' ' + self._root + ' '
            output += ')'
            return output
        else:
            return self._root
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        if isinstance(self._root, str):
            if self._root != OP_ADD and self._root != OP_MULTIPLY:
                lookup[self._root] = 0
        for tree in self._subtrees:
            tree.populate_lookup(lookup)
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    for i in range(1, len(values)):
        ...
        for j in range(len(values[i])):
            tree = ExprTree(values[i][j], [])
            t.append(tree)
            if values[i][j] == OP_ADD or values[i][j] == OP_MULTIPLY:
                q.enqueue(tree)
    return expr_tree
